...
const normalizePhrase = (phrase: string, width: number) => {
  ...
  const paddingTotal = width - safe.length;
  if (paddingTotal <= 0) return safe.slice(0, width);
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const totalDuration = plans.reduce(
        (max, plan) => plan ? Math.max(max, plan.start + plan.sequence.length * safeFlipMs) : max,
        0
      );
  ...
  return (
    ...
    >
      {tiles.map((tile, index) => {
        const isEmpty = !tile.flipping && tile.current === ' ';
        return (
          <span
            className={`split-flap-text__tile ${isEmpty ? 'split-flap-text__tile--empty' : ''}`.trim()}
            aria-hidden="true"
            key={`${index}-${tiles.length}`}
          >
            <span className="split-flap-text__half split-flap-text__half--top">
              <span className="split-flap-text__char">
                {isEmpty ? '·' : (tile.current === ' ' ? '\u00A0' : tile.current)}
              </span>
            </span>
            <span className="split-flap-text__half split-flap-text__half--bottom">
              <span className="split-flap-text__char">
                {tile.flipping ? (tile.next === ' ' ? '\u00A0' : tile.next) : (isEmpty ? '·' : tile.current)}
              </span>
            </span>
